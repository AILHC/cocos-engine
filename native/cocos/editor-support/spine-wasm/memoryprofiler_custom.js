...
var emscriptenMemoryProfiler = {
  ...
  onMemoryResize: function onMemoryResize(oldSize, newSize) {
    ...
    console.log('内存调整: ' + oldSize + ' ' + newSize);
  },
...
//      throw 'double free!';
  ...
  initialize: function initialize() {
    ...
    if (!emscriptenMemoryProfiler.memoryprofiler_summary) {
      ...
      div.innerHTML = "<div id='memoryprofiler_panel' style='position: fixed; top: 10px; right: 10px; width: 450px; max-height: 80vh; overflow-y: auto; background: rgba(255,255,255,0.98); border: 2px solid #333; border-radius: 8px; box-shadow: 0 8px 16px rgba(0,0,0,0.3); z-index: 999999; padding: 15px; display: none; font-family: monospace; font-size: 12px;'><div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;border-bottom:1px solid #ddd;padding-bottom:8px;'><strong>内存分析器</strong><button id='profiler_close_btn' style='cursor:pointer;padding:4px 10px;background:#ddd;border:1px solid #999;border-radius:3px;' onclick='document.getElementById(\"memoryprofiler_panel\").style.display=\"none\"'>−</button></div><canvas style='border: 1px solid black; margin-left: auto; margin-right: auto; display: block;' id='memoryprofiler_canvas' width='100%' height='50'></canvas><input type='checkbox' id='showHeapResizes' onclick='emscriptenMemoryProfiler.updateUi()'>显示堆内存和 sbrk() 调整。过滤关键字: <input type='text' id='sbrkFilter'>(重新打开页面时使用 ?sbrkFilter=foo,bar 查询参数预填充此列表)<br/>跟踪分配大于 <input id='memoryprofiler_min_tracked_alloc_size' type=number value="+emscriptenMemoryProfiler.trackedCallstackMinSizeBytes+"></input> 字节的分配，且多于 <input id='memoryprofiler_min_tracked_alloc_count' type=number value="+emscriptenMemoryProfiler.trackedCallstackMinAllocCount+"></input> 个未释放分配。(通过 URL 查询参数 foo.html?trackbytes=1000&trackcount=100 访问此页面，以从页面加载开始应用自定义阈值)<br/><div id='memoryprofiler_summary'></div><input id='memoryprofiler_clear_alloc_stats' type='button' value='清除统计' ></input><br />排序方式:<select id='memoryProfilerSort'><option value='bytes'>字节数</option><option value='count'>分配次数</option><option value='fixed'>固定</option></select><div id='memoryprofiler_ptrs'></div></div>";
    }
  ...
  printHeapResizeLog: function(heapResizes) {
    ...
    for (var i = 0; i < heapResizes.length; ++i) {
      ...
      var count = j - i;
      html += '<div style="background-color: ' + resizeFirst.color + '"><b>' + resizeFirst.begin + '-' + resizeLast.end + ' (' + count + ' 次, ' + emscriptenMemoryProfiler.formatBytes(resizeLast.end-resizeFirst.begin) + ')</b>:' + demangler(resizeFirst.filteredStack || resizeFirst.stack) + '</div><br>';
      i = j-1;
    }
    return html;
  },
  ...
  updateUi: function updateUi() {
    ...
    // if (document.body.style.overflow != '') document.body.style.overflow = '';  // 禁用强制滚动
    ...
    var html = '堆内存总大小: ' + self.formatBytes(HEAP8.length) + '.';
    html += '<br />' + colorBar('#202020') + '静态内存区域大小: ' + self.formatBytes(stackMax - {{{ GLOBAL_BASE }}});
    html += '. {{{ GLOBAL_BASE }}}: ' + toHex({{{ GLOBAL_BASE }}}, width);

    html += '<br />' + colorBar('#FF8080') + '栈内存区域大小: ' + self.formatBytes(stackBase - stackMax);
    html += '. STACK_BASE: ' + toHex(stackBase, width);
    html += '. STACKTOP: ' + toHex(stackCurrent, width);
    html += '. STACK_MAX: ' + toHex(stackMax, width) + '.';
    html += '<br />当前使用的栈内存 (应该为零): ' + self.formatBytes(stackBase - stackCurrent) + '.' + colorBar('#FFFF00') + ' 栈最高水位线（历史最大值，近似下界！）: ' + self.formatBytes(stackBase - self.stackTopWatermark);

    var heap_base = Module['___heap_base'];
    var heap_end = _sbrk();
    html += "<br />动态内存区域大小: " + self.formatBytes(heap_end - heap_base);
    html += ". 起始: " + toHex(heap_base, width);
    html += ". 结束: " + toHex(heap_end, width) + ".";
    html += "<br />" + colorBar("#6699CC") + colorBar("#003366") + colorBar("#0000FF") + "已使用的动态内存: " + self.formatBytes(self.totalMemoryAllocated) + " (" + (self.totalMemoryAllocated * 100 / (HEAP8.length - heap_base)).toFixed(2) + "% 的所有动态内存和未分配堆)";
    html += "<br />空闲内存: " + colorBar("#70FF70") + "动态: " + self.formatBytes(heap_end - heap_base - self.totalMemoryAllocated) + ", " + colorBar('#FFFFFF') + '未分配堆内存: ' + self.formatBytes(HEAP8.length - heap_end) + " (" + ((HEAP8.length - heap_base - self.totalMemoryAllocated) * 100 / (HEAP8.length - heap_base)).toFixed(2) + "% 的所有动态内存和未分配堆)";
    ...
    html += '<br />' + colorBar('#FF9900') + colorBar('#FFDD33') + '预加载内存使用，很可能是虚拟文件系统中文件保留的内存: ' + self.formatBytes(preloadedMemoryUsed);

    html += '<br />OpenAL 音频数据: ' + self.formatBytes(self.countOpenALAudioDataSize()) + ' (堆外)';
    html += '<br />应用生命周期内 malloc/free 总次数: ' + self.totalTimesMallocCalled + '/' + self.totalTimesFreeCalled + ' (当前存活指针: ' + (self.totalTimesMallocCalled-self.totalTimesFreeCalled) + ')';
    ...
    if (document.getElementById('showHeapResizes').checked) {
      // Print heap resize traces.
      html += '<div style="background-color: #c0c0c0"><h4>堆内存调整位置:</h4>';
      html += self.printHeapResizeLog(self.resizeMemorySources);
      html += '</div>'

      // Print heap sbrk traces.
      html += '<div style="background-color: #c0c0ff"><h4>内存 sbrk() 位置:</h4>';
      ...
      if (Object.keys(self.allocationsAtLoc).length > 0) {
        ...
        if (calls.length > 0) {
          ...
          html += '<h4>分配位置：累计超过 ' + self.formatBytes(self.trackedCallstackMinSizeBytes) + ' 字节，或同时超过 ' + self.trackedCallstackMinAllocCount + ' 个未释放分配:</h4>'
          for (var i in calls) {
            if (calls[i].length == 3) calls[i] = [calls[i][0], calls[i][1], calls[i][2], demangler(calls[i][2])];
            html += "<b>" + self.formatBytes(calls[i][1]) + '/' + calls[i][0] + " 次分配</b>: " + calls[i][3] + "<br />";
          }
        }
      }
    }
...
if (typeof document != 'undefined' && typeof window != 'undefined' && typeof process == 'undefined') {
  ...
  if (typeof document !== 'undefined' && document.body) {
    ...
    toggleBtn.textContent = '内存分析';
    toggleBtn.style.cssText = 'position:fixed;bottom:20px;right:20px;z-index:999999;padding:10px 16px;background:#4CAF50;color:white;border:none;border-radius:4px;cursor:pointer;font-size:14px;font-weight:bold;box-shadow:0 2px 5px rgba(0,0,0,0.2);transition:all 0.2s';
    toggleBtn.onmouseover = function() { this.style.transform = 'scale(1.05)'; };
    toggleBtn.onmouseout = function() { this.style.transform = 'scale(1)'; };
    toggleBtn.onclick = function() {
      var panel = document.getElementById('memoryprofiler_panel');
      if (panel) {
